...
Tabula.AddPersonView = Backbone.View.extend({
    name: "AddPersonView",
    templateHtml:
    ...
    render: function() {
    	this.$el.find('#test').html( this.template());
		return this;
	}
});

Tabula.BaseModalView = Backbone.View.extend({

    id: 'base-modal',
    className: 'modal fade hide',
    template: _.template('alert("helo")'),
    render: function(){
...
Tabula.Query = Backbone.Model.extend({
  ...
  doQuery: function(options) {
	  options.selections = this.get('list_of_coords');
    this.query_data = {
...
Tabula.ControlPanelView = Backbone.View.extend({ // only one
  ...
  queryAllData : function(){
    ...
    Tabula.pdf_view.query.doQuery({template_id:Tabula.pdf_view.pdf_document.attributes.templateName});
  },
...
function myFunction(){
	var reference = {};
	var isEntity = prompt("Is the selection an Entity?");
	if (isEntity === null) {
        return; //break out of the function early
    }
	entity_id=prompt("Enter Entity reference id.");
	reference.entity_id = entity_id;
	if(isEntity=='y'|| isEntity=='Y' || isEntity=='yes' || isEntity=='YES' || isEntity == 'Yes'){
		
	}else{
		reference.content_id=entity_id+"_content";
	}
	return reference;
}

//# sourceURL=pdf_view.js